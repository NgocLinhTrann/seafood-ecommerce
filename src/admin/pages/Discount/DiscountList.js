...
const DiscountList = () => {
    ...
    const deleteDiscount = (discount) => {
        const deleteApi = async () => {
            await axios({
                method: 'delete',
                url: `${API_DOMAIN}/api/admin/promocode`,
                data: {
                    code: discount.code,
                },
                headers: {
        ...
        toast.success(`Xóa mã khuyến mãi '${discount.code}' thành công!`);
        setDiscounts((remainDiscounts) => {
            return remainDiscounts.filter((item) => item !== discount);
        });
    };
    const confirmDelete = () => {
        if (discountToDelete) {
    ...
    useEffect(() => {
        try {
            const getApi = async () => {
                const res = await axios.get(`${API_DOMAIN}/api/admin/promocode?`, {
                    headers: {
                        Authorization: `Bearer ${info.token}`,
    ...
    useEffect(() => {
        discounts ? handleState(discounts) : console.log(discounts);
    }, [discounts]);
    ...
    return (
        <Layout title="Quản lý khuyến mãi" activePage="discount">
            ...
            <div className="w-full">
                <div className="text-2xl ml-12">Danh sách khuyến mãi</div>
                            ...
                            <div className="inline-block min-w-full py-2 sm:px-6 lg:px-8">
                                <div className="overflow-hidden">
                                    <table className="min-w-full text-left text-sm font-light">
                                        ...
                                        {isDeleteConfirmationVisible && discountToDelete && (
                                            <div className="fixed inset-0 flex items-center justify-center z-50">
                                                <div className="bg-white p-8 rounded-lg shadow-md">
                                                    <p className="text-lg font-semibold mb-4">
                                                        Bạn có chắc muốn xóa mã khuyến mãi này
                                                        không?
                                                    </p>
